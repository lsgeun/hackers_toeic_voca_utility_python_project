...
def save_tts_mp3(md_file_path, source, file_name_prefix):
    ...
    if not os.path.isdir(f"{directory_path}/0 Attachment"):
        os.system(f"mkdir \"{directory_path}/0 Attachment\"")
    # '{현재 폴더 경로}/0 Attachment/{md 파일명}/' 폴더가 존재하지 않는다면 생성
    if not os.path.isdir(f"{directory_path}/0 Attachment/{md_file_name_no_ext}"):
        os.system(f"mkdir \"{directory_path}/0 Attachment/{md_file_name_no_ext}\"")
    ...
    if not os.path.isfile(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_aust.mp3"):
        aust = gTTS(text=source, lang='en', tld='com.au', slow=False)
        aust.save(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_aust.mp3")
    if not os.path.isfile(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_uk.mp3"):
        uk = gTTS(text=source, lang='en', tld='co.uk', slow=False)
        uk.save(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_uk.mp3")
    if not os.path.isfile(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_us.mp3"):
        us = gTTS(text=source, lang='en', tld='us', slow=False)
        us.save(f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{file_name_prefix}_us.mp3")

def insert_word_tts_mp3_file_link():
    # 전역 변수 선언
    global word, word_match, search_entrance_index, insert_index, md_file_path, file_lines, directory_path, md_file_name_no_ext
    ...
    word = re.sub(r"#+?\s+?", "", word); word = re.sub(r"\n", "", word)
    
    # tts mp3 파일의 존재성 여부를 변수에 할당
    existence_of_tts_mp3_file_path = True
    
    tts_mp3_file_paths = [f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_aust.mp3", f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_uk.mp3", f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_us.mp3"]
...
def insert_example_sentence_tts_mp3_file_link():
    # 전역 변수 선언
    global example_sentence_match, example_sentence_pattern, file_lines, search_entrance_index, insert_index, md_file_path, file_lines, word, directory_path, md_file_name_no_ext
    # 예문 문자열 할당
    ...
    for i in range(line_count//2):
        ...
        tts_mp3_file_paths = [f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_example_sentence{i+1}_aust.mp3", f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_example_sentence{i+1}_uk.mp3", f"{directory_path}/0 Attachment/{md_file_name_no_ext}/{word}_example_sentence{i+1}_us.mp3"]
...
# directory path에 있는 md file 이름들
md_file_names = os.listdir(directory_path); md_file_names = [_ for _ in md_file_names if _[-3:] == ".md"]
# directory path에 있는 md file 경로들
md_file_paths = [f"{directory_path}/{_}" for _ in md_file_names]
...
for md_file_path in md_file_paths:    
    ...
    while(word_match is not None):
        ...
        word_prefix_match = re.search(r"^#{2,}", word)
        # word가 하위 단어일 동안 반복
        while(word_prefix_match is not None):
            ...
            # word_pattern으로 찾은 word가 하위 단어인지 검사하는 word_prefix_match 변수 정의
            word_prefix_match = re.search("^#{2,}", word)
